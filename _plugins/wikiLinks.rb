...
module Jekyll
  ...
  class WikiLinks < Generator
      ...
      def initialize(config = {})
        ...
        raise 'Missing wiki configuration. Please configure wiki parameters in _config.yml' unless config['buildDocs']
        @conf = config['buildDocs']
      ...
      def generate(site)
        ...
        wiki_pages.each do |p|
          ...
          jekyll_url = @conf['baseurl'] + '/' + @conf['buildDocs']['wiki_dest'] + '/' + p.basename + '.html'
      ...
      def getWikiRepositoryUrl(full = true)
        ...
        if @conf['buildDocs']['wiki_repository_url']
          url = @conf['buildDocs']['wiki_repository_url'].sub('.wiki.git', '') + '/wiki'
        end
